<template>
  <div class="min-h-screen flex p-5 lg:p-10 xl:p-20" >
    ...
    <main class="p-5 w-full min-h-screen lg:px-4">
      ...
      <div class="flex my-5 md:mt-10">
        <div class="md:w-2/5" aria-hidden="true"></div>
        <div class="w-full md:w-3/5 flex justify-end gap-5 ">
          <BaseSelect selected="商品排序" :options="sortOptions"></BaseSelect>
          <BaseSelect selected="每頁筆數" :options="perPageOptions"></BaseSelect>
        </div>
      </div>
      <div class="grid gap-5 xs:grid-cols-2 xs:gap-3 lg:grid-cols-3  lg:gap-5">
        <ProductCard v-for="product in products" :key="product.id" :productCard="product"/>
      </div>
